class User < ApplicationRecord
  ...
  has_many :invited_events, through: :invites, source: :event
  has_many :accepted,  -> { where(accepted: true) },class_name: "Invitation", foreign_key: :invitee_id
  has_many :pending,  -> { where(accepted: false) },class_name: "Invitation", foreign_key: :invitee_id
  #has_many :accepted_events, -> { where(accepted: false) }through: :invites, -> { where accepted: true}, source: :event
  has_many :accepted_events, through: :accepted, source: :event
  has_many :pending_events, through: :pending, source: :event
